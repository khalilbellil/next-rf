...
module.exports = async (req, res) => {
    ...
    if(id_user){
        try {
            ...
            }else if(id_project && one === 'zip' || one === 'id_department' || one === 'id_city' || one === 'id_region' || one === 'address'){
                const project = await db.query(`SELECT id_address FROM project WHERE id = '${id_project}' LIMIT 1`)
                const ress = await db.query(`UPDATE address SET address = '${one_val}' WHERE id = '${project[0].id_address}'`)
                if(ress.affectedRows > 0){
                    success = 'yes'
                }
            }
